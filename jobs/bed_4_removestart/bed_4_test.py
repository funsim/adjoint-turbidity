...
filter_2 = e**-(equation.smooth_pos((target.get_data_x(type)[0] + 1000) - model.y*x_N*model.h_0*h_0_norm))
...
def prep_target_cb(model):
  ...
  y, q, h, phi, phi_d, x_N, u_N, k, phi_int = \
        input_output.map_to_arrays(model.w[0], model.y, model.mesh)
  info_red("%f"%x_N)
  info_red("%f"%(target.get_data_x(type)[0] + 1000))

J_integral = scale**-1*filter*filter_2*inner(diff, diff)
