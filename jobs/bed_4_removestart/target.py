...
phi_d_x = [data[0][4:,0], data[1][4:,0]]
phi_d_y = [data[0][4:,1], data[1][4:,1]]
...
if __name__=='__main__':
  ...
  filter_2 = e**-(equation.smooth_pos((phi_d_x[type][0] + 1000) - y*x_N))
  f = Function(fs)
  solve(v*filter_2*d*dx - v*f*dx == 0, f)
  ...
  v = TestFunction(R)
  scale = Function(R)
  scale_f = v*filter*filter_2*dx - v*scale*dx
  solve(scale_f == 0, scale)
  print scale.vector().array()[0]

  scaled_f = Function(fs)
  v = TestFunction(fs)
  solve(v*scale**-1*filter*filter_2*f*dx - v*scaled_f*dx == 0, scaled_f) 
